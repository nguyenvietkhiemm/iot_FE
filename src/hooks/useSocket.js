...
const useSocket = () => {
  ...
  useEffect(() => {
    fetchDataDevices();
    const fetchDataInterval = setInterval(fetchDataSensors, 2000); // Gọi API mỗi 1 giây
  ...
  const sendControlData = async (controlData) => {
    try {
      const response = await axios.post(`http://localhost:3000/realtime/api/control/device`, controlData);
